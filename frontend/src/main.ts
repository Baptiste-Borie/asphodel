...
function renderEmptyLibrary(): void {
  ...
  description.textContent =
    "Importez une decklist pour créer votre premier deck persistant.";
...
function renderDeckDetail(deck: DeckDetail): void {
  ...
  const mainboardCards = deck.cards.filter(
    (card) => card.section === "mainboard",
  );

  deckDetail.append(heading);
  if (commanderCards.length > 0) {
    deckDetail.append(createDeckSection("Commander", commanderCards, true));
  }
  if (mainboardCards.length > 0) {
    deckDetail.append(createDeckSection("Mainboard", mainboardCards));
  }
}
...
async function openDeck(id: number): Promise<void> {
  ...
  try {
    ...
    deckDetail.textContent =
      error instanceof Error ? error.message : "Impossible d’ouvrir ce deck.";
  }
}
...
function renderImportError(error: ApiError): void {
  ...
  const items = [
    ...(error.payload.issues?.map(
      (issue) => `Ligne ${issue.line} : ${issue.message} (${issue.content})`,
    ) ?? []),
    ...(error.payload.cardNames?.map(
      (cardName) => `Carte introuvable : ${cardName}`,
    ) ?? []),
  ];
...
deckForm.addEventListener("submit", async (event) => {
  ...
  importFeedback.textContent =
    "Résolution des cartes avec Scryfall et enregistrement…";
  importFeedback.hidden = false;
  ...
  try {
    ...
    renderImportError(
      error instanceof ApiError
        ? error
        : new ApiError({ message: "Le backend est inaccessible." }),
    );
  } finally {
...
document.querySelectorAll<HTMLButtonElement>("[data-open-import]").forEach(
  (button) => button.addEventListener("click", openImportModal),
);
element<HTMLButtonElement>("#close-deck-modal").addEventListener(
  "click",
  closeImportModal,
);
element<HTMLButtonElement>("#cancel-deck-modal").addEventListener(
  "click",
  closeImportModal,
);
element<HTMLButtonElement>("#back-to-library").addEventListener("click", showLibrary);
element<HTMLButtonElement>("#home-button").addEventListener("click", showLibrary);
